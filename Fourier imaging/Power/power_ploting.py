...
output_dir = os.path.join(repo_root, 'Fourier imaging', 'Power', 'Fourier_Images')
...
for day, df in enumerate(dataframes):
    ...
    for idx, row in df.iterrows():
        # Sample ID for naming contain the info if its healthy or unhealthy
        ...
        plt.imshow(np.log1p(power), cmap='hot', aspect='auto')
        plt.axis('off')
        plt.savefig(os.path.join(day_dir, f'{sample_id}_power.png'))
        plt.close()